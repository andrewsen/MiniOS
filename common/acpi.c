...
static void init_root_tables(void) {
  rsdp = (struct acpi_rsdp*)memmem((void*)EBDA_ADDR, AFTER_EBDA_ADDR - EBDA_ADDR, RSDP_SIGNATURE, 8);
  if (!rsdp)
    rsdp = (struct acpi_rsdp*)memmem((void*)BIOS_ROM_ADDR, AFTER_BIOS_ROM_ADDR - BIOS_ROM_ADDR, RSDP_SIGNATURE, 8);
