 ...
 *
 * Хвала Аллаху, все типы кроме double и (u)long занимают 4 байта
 *
 * byte x byte => NSC
 * short x short =>
 *
 ...
 *  - "Грязные" регистры. Пример: для команд div/idiv регистр edx должен равняться нулю чтобы избежать SIGFPE.
...
byte* Runtime::jitCompile(Function *f)
{
    ...
    while(code < f->bytecode + f->bc_size) {
        ...
        switch (*code) {
            ...
            case OpCode::LD_F:
            {
                ...
                panic("JIT: LD_F unsupported", 2, -1);
                return;
            }
                break;
            case OpCode::LD_UI8:
            ...
            case OpCode::LD_I64:
            {
                panic("JIT: LD_I64 unsupported", 2, -1);
                return;
            }

            case OpCode::LD_UI64:
            {
                panic("JIT: LD_UI64 unsupported", 2, -1);
                return;
            }
            case OpCode::LD_STR:
            {
            ...
            case OpCode::SHR: {
                ...
                break;
            case OpCode::SIZEOF: {
                panic("SIZEOF is not implemented", 2, -1);
                return;
            }
                break;
    ...
    byte* data = (byte*)kmalloc(x86code.size()+1);
