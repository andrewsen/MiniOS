...
Runtime::Runtime()
{
    ...
    Instance = nullptr;
    function_list.name = "func_vector";
    //Instance = this;
    //callsta.reserve(50);
}
...
extern "C" void _Zdl() // FUCK OFF, GCC :*
{
    // It's so lol
}
...
void Runtime::Create()
{
    ...
    program_stack = (byte*)calloc(current_stack_size, 1);//new byte[current_stack_size]; //#OLDNEW
...
Function* Runtime::FindFunction(const char *name, RTType args[], uint count) const { ///TODO: FIX OPTIMIZE
//main_module.included
    for(uint i = 0; i < main_module.func_count; ++i)
    {
        //cout << "Comparing " << name << " with `" << main_module.functions[i]->sign << endl;
        if(!strcmp(main_module.functions[i]->sign, name))
        {
            //cout << "Args " << count << " with " << main_module.functions[i]->argc << endl;
            if(main_module.functions[i]->argc == count)
            {
                auto fun = main_module.functions[i];
                for(uint t = 0; t < count; ++t)
                {
                    //cout << "Types " << (uint)args[t].plain << " with " << (uint)(*fun->args[t].type) << endl;
                    if(args[t].dimens != 0)
                    {
                        if((*fun->args[t].type != (byte)Type::ARRAY)
                           || (*(uint*)(fun->args[t].type+1) != args[t].dimens)
                ...
                return main_module.functions[i];
            }
        }
    }
    return nullptr;
...
void Runtime::rtThrowEx(const char* file, int line, Runtime::RtExType t, string message)
{
    ...
    printStackTrace();
    //this->Unload();
    panic("Exception was thrown", 2, Runtime::_EXIT_FAILURE);
}

void Runtime::Call(Function *next)
{
    /*
...
void Runtime::execFunction(Function *f, byte* fargs, byte* local_table)
{
    ...
    while(true) {
        ...
        switch (*code) {
            ...
            default:
                log.SetType(Log::Error);
                log << "\nUnknown instruction " << OpcodeToStr(*code) << "\n";
                log << "  address: " << (uint)(code - f->bytecode) << "\n";
                log << "Call stack:\n";
                //printStackTrace("  at: ");
